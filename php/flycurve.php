...
function yieldcurve1(string $c)
{
    ...
    {
        $conn = OpenConnection();
        $tsql = "SELECT DISTINCT contract, MAX(soff3) as soff3, MAX(CONVERT(DateTime, date, 103)) from {$c}_test2 WHERE soff3 NOT LIKE 'NULL' AND CONVERT(datetime, date, 103) >= '$yesterday' GROUP BY contract";
...
function yieldcurve2(string $c, string $d)
{
    ...
    {
        $conn = OpenConnection();
        $tsql = "SELECT DISTINCT contract, MAX(soff3) as soff3, MAX(CONVERT(DateTime, date, 103)) from {$d}_test2 WHERE soff3 NOT LIKE 'NULL' AND CONVERT(datetime, date, 103) >= '$yesterday' GROUP BY contract";
        $getInfo = sqlsrv_query($conn, $tsql);
        if ($getInfo == FALSE)
            die(FormatErrors(sqlsrv_errors()));
        $contractInfoCount = 0;
...
function yieldcurve3(string $c, string $d, string $e)
{
    ...
    {
        $conn = OpenConnection();
        $tsql = "SELECT DISTINCT contract, MAX(soff3) as soff3, MAX(CONVERT(DateTime, date, 103)) from {$e}_test2 WHERE soff3 NOT LIKE 'NULL' AND CONVERT(datetime, date, 103) >= '$yesterday' GROUP BY contract";
        $getInfo = sqlsrv_query($conn, $tsql);
        if ($getInfo == FALSE)
            die(FormatErrors(sqlsrv_errors()));
        $contractInfoCount = 0;
